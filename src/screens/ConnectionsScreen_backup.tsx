...
interface Follower extends BaseConnection {
...
export default function ConnectionsScreen({ route }: { route: any }) {
  ...
  const isFollower = (connection: Connection): connection is Follower => {
    return 'mutualConnections' in connection;
  };

  const isFollowing = (connection: Connection): connection is Following => {
    return 'isFollowing' in connection;
  };
              ...
              {activeTab === 'followers' ? (
                ...
                <View style={styles.actionButtons}>
                  <TouchableOpacity 
                    style={[
                      styles.followButton, 
                      isFollowing(connection) && connection.isFollowing && styles.followingButton
                    ]}
                    onPress={() => handleFollowToggle(connection.id)}
                  >
                    <Text style={[
                      styles.followButtonText, 
                      isFollowing(connection) && connection.isFollowing && styles.followingButtonText
                    ]}>
                      {isFollowing(connection) && connection.isFollowing ? 'Following' : 'Follow'}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.messageButton}>
                    <Ionicons name="chatbubble" size={16} color="#6B7280" />
